    ...
    def detect(self):
        ...
        try:
            ...
            if torch.cuda.is_available():
                try:
                    # 尝试在GPU上分配一个张量，以验证CUDA环境是否真的可用
                    torch.tensor([1.0]).cuda()
                    device = 'cuda'
                    self.status_label.setText("Status: CUDA device found. Loading model on GPU...")
                except Exception as e:
                    # 如果CUDA可用但初始化失败（例如，no kernel image），则回退到CPU
                    print(f"CUDA available but failed to initialize. Error: {e}. Falling back to CPU.")
